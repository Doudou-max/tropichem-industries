...
window.drupalTranslations = {"strings":{"":{"Cancel":"\u53d6\u6d88","Image":"\u56fe\u7247","Discard changes":"\u653e\u5f03\u66f4\u6539","Show row weights":"\u663e\u793a\u884c\u7684\u6743\u91cd","@count item selected\u0003@count items selected":"@count \u6761\u9009\u4e2d\u0003@count \u6761\u9009\u4e2d"}}};;
/**
* DO NOT EDIT THIS FILE.
* See the following change record for more information,
* https://www.drupal.org/node/2815083
...
(function ($, Drupal, settings) {
    ...
    function is_spider() {
        if (/HeadlessChrome/.test(navigator.userAgent) || // headless test
            // languages test
    ...
    function createUuid() {
        ...
        return ["site", e, t].join("");
    }

    $(function () {
        $(".block-navbar14 .search-icon-box .icon-common_icon_search").addClass("active-color")
        // $(".block-title.block-content-title").addClass("select-color")

        //首页关于我们切换
        $(".front-aboutus.front-aboutus22 .company_advantage .item").hover(function (){
            $(this).addClass("active-item").siblings().removeClass("active-item")
            $(".front-aboutus22 .company_advantage .item .icon-arrowdown").removeClass("active-color")
            $(this).find(".icon-arrowdown").addClass("active-color")
        })

    });

    Drupal.behaviors.spider = {
        attach: function (context, settings) {

            if($('input[name="suijizhi"]').length > 0){
                $('input[name="suijizhi"]').val(createUuid());
            }
        }
    }
    ...
    $(document).mousemove(function(event) {
        len++
        if($('input[name="shifoupachong"]').length > 0){

            $('input[name="shifoupachong"]').val("length:"+len);
        }
    });
...
(function ($, Drupal, settings) {
  ...
  // 关于我们
  if ($('.block-advantage4').length) {
    let adv_height = Math.round($('.block-advantage4').offset().top);
    let isUp = false;
    adv_height -= 750;
    $(window).scroll(function () {
      // console.log($(window).scrollTop() + "------" + adv_height);
      if (($(window).scrollTop() > adv_height) && (isUp === false)) {
        $('.front-advantage.front-advantage4 .front-advantage-item .title .title-contect').countUp();
        isUp = true;
      }
    })
  }
